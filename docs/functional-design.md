...
| フック名 | トリガー | 実行内容 |
|---------|---------|---------|
| `spec-start` | `.steering/` 配下に新ディレクトリが作成された時 | requirements.md の記入ガイドをエージェントに通知 |
| `tasklist-update` | `tasklist.md` が更新された時 | 未完了タスクの確認を促す |

> **注意**: フック機能の具体的な API・トリガー種別は Kiro のバージョンに依存する。アーキテクチャ設計フェーズで実現可能性を確認する。
